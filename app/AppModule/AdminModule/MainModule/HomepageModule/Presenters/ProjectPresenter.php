...
class ProjectPresenter extends \App\AppModule\AdminModule\MainModule\BasePresenter
{
	...
	public function __construct(
		...
		private \App\AppModule\AdminModule\MainModule\HomepageModule\Forms\Project\AddFormFactory   $addFormFactory,
		private \App\AppModule\AdminModule\MainModule\HomepageModule\Forms\Project\EditFormFactory  $editFormFactory,
	...
	public function renderDefault(?int $id)
	{
		if ($id) {
			...
			if (isset($_FILES['file_to_upload']['tmp_name']) && $_FILES['file_to_upload']['tmp_name']) {
				...
				if (!is_dir("docs/homepage/projects/{$id}")) {
					mkdir("docs/homepage/projects/{$id}",0777, true);
				}
				if (!$file_temp_location) {
					$this->flashMessage('ERROR: No file has been selected', 'danger');
				}  elseif (move_uploaded_file($file_temp_location, "docs/homepage/projects/{$id}/$file_name")){
	...
	public function actionDelete(int $id)
	{
		...
		$this->cacheService->removeKey('projects-' . $model->get('lang')->getValue());
				...
				'columns' => [
					'photo' => [
						...
						'render' => function($record, $photo) {
							return "<img style='width:100px;height:100px;border-radius:0;' src='/"
								. ($this->settingsService->getAppConfig()['subdir']
									. ($record->get($photo->getName())->getValue()
										? "docs/homepage/projects/".$record->get('id')->getValue() ."/" .$record->get($photo->getName())->getValue()
